...
def vertical_win
    ...
    if (index.even? or index == 0) 
        value.each_with_index do |v, i|   
            ...
            # if  add <= 13 and $board1[index][add]
        ...
        if (index.even? or index == 0) 
            value.each_with_index do |v, i|   
                ...
                when "   x   "   
                    ...
                    daigonal_index = index + 2
                    diagonal_i = i + 2  
                    
                    
                    $board1[diagonal_index][diagonal_i].include?("   x   ")
                
                # add = i + 2
            
                # if  add <= 13 and $board1[index][add]
    
                if  daigonal_i <= 12 and $board1[diagonal_index][diagonal_i].include?("   x   ")
                    true_count += 1 
                    puts "#{i} and  #{index}"
                    puts "true count = #{true_count}"    
